...
class ImageSpace(BaseSpace):
    ...
    def minimal_enclosing(cls, surfs, reference):
        ...
        Args:
            surfs: singular or list of surface objects in world-mm coords
        ...
        min_vox, max_vox = utils.round_voxel_coordinates(min_max)
        size = max_vox - min_vox + 1

        # Get a copy of the corresponding mm coords for checking later
        min_max_mm = utils.affine_transform(
            np.array([min_vox, max_vox]), space.vox2world
        )
        ...
        for surf in slist:
            if not utils.space_encloses_surface(space, surf):
        ...
        if not (new_voxs.size and old_voxs.size):
            warnings.warn(
                f"Surfaces {[s.name for s in slist]} do not intersect the reference voxel grid"
            )
    ...
    def derives_from(self, parent):
        ...
        return (np.abs(det1 - det2) < 1e-9) and np.all(
            np.abs(offset_mm - offset_mm2) < 1e9
        )
...
def reindexing_filter(src_space, dest_space, as_bool=False):
    ...
    fltr = np.logical_and(
        np.all(voxs_in_dest > -1, 1), np.all(voxs_in_dest < dest_space.size, 1)
    )

    src_inds = np.ravel_multi_index(voxs_in_src[fltr, :].T, size)
    dest_inds = np.ravel_multi_index(voxs_in_dest[fltr, :].T, dest_space.size)
