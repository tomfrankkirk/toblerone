...
from toblerone.ctoblerone import (
    _ctestTriangleVoxelIntersection,
    _cyfilterTriangles,
...
TETRA1 = np.array(
    [[0, 3, 4, 5], [0, 1, 2, 4], [0, 2, 4, 5]], dtype=np.int32  # aABC  # abcB  # acBC
)

TETRA2 = np.array(
    [[0, 3, 4, 5], [0, 1, 2, 5], [0, 1, 4, 5]], dtype=np.int32  # aABC  # abcC  # abBC
)
...
def _pointGroupsIntersect(grps, tris):
    ...
    for g in range(len(grps)):
        for h in range(g + 1, len(grps)):
            if np.any(np.intersect1d(tris[grps[g], :], tris[grps[h], :])):
                return True
        ...
        while _pointGroupsIntersect(groups, tris):
            ...
            for g in range(len(groups)):
                ...
                for h in range(g + 1, len(groups)):
                    ...
                    if np.any(np.intersect1d(tris[groups[g], :], tris[groups[h], :])):
...
def form_associations(points_vox, tris, space, cores=mp.cpu_count()):
    ...
    workerFunc = functools.partial(
        _formAssociationsWorker, tris, points_vox, space.size
    )
...
def _findRayTriangleIntersections2D(testPnt, patch, axis):
    ...
    fltr = _cytestManyRayTriangleIntersections(
        patch.tris, patch.points, testPnt, (axis + 1) % 3, (axis + 2) % 3
    )

    # And find the multipliers for those that do intersect
    if np.any(fltr):
        mus = _findRayTriPlaneIntersections(
            patch.points[patch.tris[fltr, 0], :], patch.xprods[fltr, :], testPnt, ray
        )
    else:
        mus = np.array([])
...
def _findRayTriangleIntersections3D(testPnt, ray, patch):
    ...
    onPlane = patch.points - np.einsum("i,j->ij", lmbda, ray, casting="no") - testPnt
    ...
    onPlane2d = np.array(
        [
            np.einsum("ij,j->i", onPlane, d1, casting="no"),
    ...
    fltr = _cytestManyRayTriangleIntersections(patch.tris, onPlane2d.T, ZERO_3, 0, 1)
...
def _fullRayIntersectionTest(testPnt, surf, voxIJK, size):
    ...
    inds = np.ravel_multi_index((subs[:, 0], subs[:, 1], subs[:, 2]), size)
...
def _classifyVoxelViaRecursion(patch, voxCent, vox_size, containedFlag):
    ...
    flags = _reducedRayIntersectionTest(subVoxCents, patch, voxCent, ~containedFlag)

    return flags.sum() / SUPER2_DIV


def _estimateVoxelFraction(surf, voxIJK, voxIdx, supr):
    """The Big Daddy that does the Heavy Lifting.
    Recursive estimation of PVs within a single voxel. Overview as follows:
    - split voxel into subvoxels according to supr
    ...
    subvox_size = (1.0 / supr).astype(NP_FLOAT)
    ...
    allCents = _get_subvoxel_grid(supr) + voxIJK
    allCentFlags = _reducedRayIntersectionTest(allCents, patch, voxIJK, ~voxCentFlag)

    # Subvoxel loop starts here -----------------------------------------------

    for s in range(supr.prod()):
        ...
        triFltr = _cyfilterTriangles(patch.tris, patch.points, subVoxCent, subvox_size)
        ...
        # CASE 2: some triangles intersect the subvox -------------------------
            ...
            cornerFlags = _reducedRayIntersectionTest(
                corners, patch, voxIJK, ~voxCentFlag
            )
            ...
            edgeIntXs, fold = _findVoxelSurfaceIntersections(smallPatch, corners)
            ...
            if (not fold) & (len(groups) < 2):
                ...
                localPs = localPs[_filterPoints(localPs, subVoxCent, subvox_size)]
                ...
                hullPts = np.vstack(
                    (
                        localPs,
                        edgeIntXs,
                        _findTriangleVoxFaceIntersections(
                            smallPatch, subVoxCent, subvox_size
                        ),
                    )
                )

                if verbose:
                    print("Hulls: mixed in/out")
                ...
                # Aim to form the smallest possible hull
                    ...
                    msh = trimesh.base.Trimesh(
                        vertices=smallPatch.points, faces=smallPatch.tris
                    )
                ...
                if not V:
                    L2fraction = _classifyVoxelViaRecursion(
                        smallPatch, subVoxCent, subvox_size, subVoxFlag
                    )
...
def _estimateFractions(surf, supr, descriptor, cores):
    ...
    Args:
        ...
        supr: 1 x 3 vector of supersampling factor
    Returns:
        vector of size prod(FoV)
    """

    supr = np.squeeze(np.array(supr, dtype=np.int16))

    # Compute all voxel centres, prepare a partial function application for
    # use with the parallel pool map function
    workerChunks = utils._distributeObjects(range(surf.indexed.assocs_keys.size), 50)
    estimatePartial = functools.partial(_estimateFractionsWorker, surf, supr)
...
def _estimateFractionsWorker(surf, supr, chunk):
    ...
    try:
        ...
        vox_ijks = np.array(
            np.unravel_index(vox_inds, surf.indexed.space.size), dtype=NP_FLOAT
        ).T

        for idx in range(len(chunk)):
            pvs[idx] = _estimateVoxelFraction(
                surf, vox_ijks[idx, :], vox_inds[idx], supr
            )
...
def _voxelise_worker(surf, dim_range, raysd1d2):
    ...
    if not raysd1d2.size:
        ...
        for d1d2 in raysd1d2:
            ...
            intersectionMus = _findRayTriangleIntersections2D(start_point, patches, dim)
            ...
            if intersectionMus.shape[0] % 2:
                raise RuntimeError(
                    "voxelise: odd number of intersections"
...
def vox_tri_weights(
    in_surf, out_surf, spc, factor, cores=mp.cpu_count(), descriptor="", ones=False
):
    """
    ...
    worker = functools.partial(
        _vox_tri_weights_worker,
        inps_vox=points_vox[0],
        outps_vox=points_vox[1],
        tris=in_surf.tris,
        spc=spc,
        factor=factor,
        ones=ones,
...
def _vox_tri_weights_worker(
    t_range, inps_vox, outps_vox, tris, spc, factor, ones=False
):
    """
    ...
    vox_tri_samps = sparse.dok_matrix((spc.n_vox, tris.shape[0]), dtype=NP_FLOAT)
    samplers = [np.linspace(0, 1, 2 * f + 1, dtype=NP_FLOAT)[1:-1:2] for f in factor]
...
def __meyer_worker(points, tris, edges, edge_lengths, worklist):
    ...
    vtx_tri_areas = sparse.dok_matrix((points.shape[0], tris.shape[0]), dtype=NP_FLOAT)
...
def vtx_tri_weights(surf, cores=mp.cpu_count()):
    ...
    worker_func = functools.partial(__meyer_worker, points, tris, edges, edge_lengths)
